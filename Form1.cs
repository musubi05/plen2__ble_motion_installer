...
namespace BLEMotionInstaller
{
    public partial class Form1 : Form
    {
            ...
            using (OpenFileDialog of = new OpenFileDialog())
            {
                ...
                foreach (string fileName in of.FileNames)
                {
                    ...
                    try
                    {
                        ...
                        {
                            // モーションファイル（XML形式)
                            PLEN.MFX.TagMfxModel tagMfx;
                            /*----- XML読み出し -----*/
                            try
                            {
                                System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(typeof(PLEN.MFX.TagMfxModel));
                                tagMfx = (PLEN.MFX.TagMfxModel)serializer.Deserialize(stream);
                            }
                            catch (Exception ex)
                            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (listBox1.SelectedItems.Count >= 1)
            {
                ...
                foreach (string portDictKey in listBox1.SelectedItems)
                {
                    ...
                    // シリアル通信スレッド用インスタンス作成
                    // イベント登録
                    portInstanceDict.Add(portName, new SerialCommProcess(portName, sendMfxCommandList));
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            threadDict.Clear();
            MessageBox.Show("全通信を中断しました。", "通信を中断しました", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
